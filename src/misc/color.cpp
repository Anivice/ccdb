...
namespace sim
{
    ...
    NumPack_t simulation_rainbow(const Num x)
    {
        ...
        if (!init_cache_)
        {
            if (customized_color_command_calc.find(utils::dump_start_signature) == std::string::npos)
            {
                bool skip = false;
                ...
                if (!skip && color_scheme == CUSTOMIZED)
                {
                    ...
                    if (!scheme_hash.empty())
                    {
                            ...
                            !std::filesystem::exists(cache_loc))
                        {
                            ...
                            color::local_color_cache.reserve(estimated_capacity);
                            ...
                            std::ranges::sort(color::local_color_cache,
                            ...
                            const auto [u_beg, u_end ] = std::ranges::unique(color::local_color_cache);
                            color::local_color_cache.erase(u_beg, u_end);
            ...
            {
                std::unique_ptr < std::istringstream > oss_ptr;
                {
                    std::istringstream oss { customized_color_command_calc };
                    const auto binaryCache = utils::importBinary(oss);
                    const auto decompressedCache = utils::decompress(binaryCache);
                    const std::string ss_str {
                        reinterpret_cast<const char*>(decompressedCache.data()),
                        reinterpret_cast<const char*>(decompressedCache.data() + decompressedCache.size()) };
                    oss_ptr = std::make_unique<std::istringstream>(ss_str);
                }
        ...
        if (color_scheme == CUSTOMIZED && !color::local_color_cache.empty()) // estimate color based on the cache
        {
            auto ratio = (x - Begin) / Span;
            if (ratio > 1.00) ratio = 1.00;
            const auto size = static_cast<int64_t>(color::local_color_cache.size());
