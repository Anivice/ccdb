...
namespace
{
    ...
    void init_backtrace(const bool quiet)
    {
        ...
        if (!quiet) utils::print("Loaded ", symbol_table.size(), " symbols from the symbolic file.\n");
    ...
    std::string testBackend(const std::string & backend, const std::string & token)
    {
        try {
    ...
    int get_subinfo(std::string sub_url, const int timeout, const std::string & header)
    {
...
int main_(int argc, char ** argv)
{
    try
    {
            ...
            CCDB_SYNC_ADDRESS_BIND_TO.empty())
        {
                ...
                utils::parse_url(backend, scheme, host, path))
            {
                ...
                if (!dev.empty()) {
                    ...
                    if (!quiet) utils::print("Setting CCDB_SYNC_ADDRESS_BIND_TO=", dev, " since backend is reachable from here.\n");
                }
            } else {
        ...
        if (const auto backend_name = testBackend(backend, token); !backend_name.empty()) {
            if (!quiet) utils::print("Backend is ", backend_name, "\n");
        } else {
